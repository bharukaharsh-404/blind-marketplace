...
import type { PublicOrder as _PublicOrder, UserProfile as _UserProfile, UserRole as _UserRole } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async getMyOrders(): Promise<Array<PublicOrder>> {
        if (this.processError) {
            try {
    ...
    async getMyPseudonym(): Promise<string | null> {
        if (this.processError) {
            try {
                const result = await this.actor.getMyPseudonym();
    ...
    async getOrderById(arg0: string): Promise<PublicOrder | null> {
        if (this.processError) {
            try {
    ...
    async getOrders(): Promise<Array<PublicOrder>> {
        if (this.processError) {
            try {
...
function from_candid_opt_n7(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_PublicOrder]): PublicOrder | null {
    return value.length === 0 ? null : value[0];
}
